...
description: >
  ...
  触发：/ppt-html-design、"做个落地页"、"设计一张海报/封面"、"做张幻灯片/PPT"、"做个交互原型"、"做个动画"、
...
<script>
  ...
  const sprites = Array.from(document.querySelectorAll(".sprite")).map(
    (el) => ({
      el,
  ...
  function tick(now) {
    ...
    currentTime = Math.min(
      currentTime + ((now - lastTick) / 1000) * speed,
      DURATION,
    );
    lastTick = now;
...
.fade-in {
  animation: spIn 0.8s cubic-bezier(0.2, 0.9, 0.3, 1) forwards;
}
.fade-out {
  animation: spOut 0.5s cubic-bezier(0.4, 0, 1, 1) forwards;
}
@keyframes spIn {
  from {
    opacity: 0;
...
document
  ...
  .addEventListener("click", () => document.body.classList.toggle("clean"));
...
document
  .getElementById("tSpeed")
  .addEventListener("input", (e) => (speed = +e.target.value));
document
  .getElementById("tAccent")
  .addEventListener("input", (e) =>
    document.documentElement.style.setProperty("--a1", e.target.value),
  );
document
  .getElementById("tAccent2")
  .addEventListener("input", (e) =>
    document.documentElement.style.setProperty("--a2", e.target.value),
  );
```
